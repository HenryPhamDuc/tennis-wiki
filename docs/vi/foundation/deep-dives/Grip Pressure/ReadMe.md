...
| Step | What happened |
|---|---|
| Read `tennis-huấn luyện viên-system` skill v1.3.0; surveyed `Deep Dives/`; re-read `Foundations & Grip.md` to find the "3/10 swing, 9/10 impact" hook the file already planted but did not develop; loaded `vietnamese-sound-cues.md` to keep onomatopoeia canonical. |
| Identified cách cầm vợt pressure as the missing dial between cách cầm vợt choice (Continental / Eastern Semi-Western / Grip Change Map) and stroke execution (the 5-stroke family). Extracted the 50+ tendon-protection angle and the sound-diagnosis channel. |
...
|---|---|---|---|
| Delỗi peak pressure per stroke | Cú Thuận Tay 8/10, Vôlei 7/10, Phát Bóng 9/10, block return 7/10 | "Squeeze more to hit harder" trope | 50+ tendons cannot afford cách cầm vợt above 9 even at peak; the L-angle and gravity do the heavy lifting. |
| Volume of strokes in the map | 16 (cú đánh nềns + Vôlei variants + return variants + Phát Bóng phases + smash + lob variants) | Just "cú đánh nềns and vôleis" | A 50+ player in Surrey hits every one of these; mapping them is the cheat sheet's job. |
...
| Continental pressure for Vôlei/cắt/Phát Bóng | `Deep Dives/Continental Grip/` |
| Eastern Semi-Western pressure for Cú Thuận Tay | `Deep Dives/Eastern Semi-Western Grip/` |
| When to change cách cầm vợt between strokes | `Deep Dives/Grip Change Map/` |
...
|---|---|---|
| A | Build the Grip Pressure Web App — interactive HTML where you click a stroke, see the swing→impact ramp animated, and hear the sound (pực/bộp/bịch). | Reinforces the dial concept kinesthetically; lives next to the Compléte Manual HTML. |
| B | Wrist Mobility & 50+ Longevity deep dive — the joint the cách cầm vợt pressure protects. Forearm stretches, eccentric exercises, the cortisone-prevention checklist. | Natural follow-up: pressure is the dial, mobility is the joint it protects. |
| C | Pressure Calibration Log — a one-page printable the user fills in after each trận: "Cú Thuận Tay today: 5/7, Vôlei: 2/6, Phát Bóng: 4/8 — felt tight on the cắt." | Turns the cheat sheet from passive Tham khảo to active self-huấn luyện viêning. |
| D | Cross-Tham khảo patch — go through all 19 existing deep dives and tag any sentence that mentions cách cầm vợt pressure with a `→ see Grip Pressure/` link. | Makes the dial visible across the library without rewriting any file. |
| E | Stop here. The dial is documented; the cheat sheet is in the bag. | Sometimes the best move is to lét it settle before adding more. |
## Files
